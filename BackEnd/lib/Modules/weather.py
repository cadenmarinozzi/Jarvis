...
appid = configuration["weatherApi"]["appId"];

def weather(textToSpeech, phrases, text, server):
    textString = "";
...
def temperature(textToSpeech, phrases, text, server):
    textString = "";
...
def airPressure(textToSpeech, phrases, text, server):
    textString = "";
...
def humidity(textToSpeech, phrases, text, server):
    textString = "";
...
def windSpeed(textToSpeech, phrases, text, server):
    textString = "";
    ...
    if (responseCode == "404"):
        speechString = "Sorry I cannot find the wind speed for " + city;
        textToSpeech(speechString);
    else:
        wind = responseJson["wind"];
        windSpeed = str(int(wind["speed"]));
        speechString = "The current wind speed in " + city + " is " + windSpeed + " miles per hour";
        textToSpeech(speechString);